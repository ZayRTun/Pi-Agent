...
import { cjkBreakRegex, getGraphemeSegmenter, getWordSegmenter, isWhitespaceChar, sliceByColumn, truncateToWidth, visibleWidth, } from "../utils.js";
...
export class Editor {
    ...
    borderColor;
    // Prime Agent TUI: optional dark surface block + popover bg + slash-command color.
    backgroundColor;
    ...
    /** Prime Agent TUI: prompt prefix rendered before line 0 (default "> "). */
    getPromptPrefix() {
    ...
    /** Prime Agent TUI: color a leading slash-command token with the accent color. */
    styleDisplayText(displayText, layoutLineIndex, lineText, cursorCol) {
        ...
        if (cursorCol !== undefined && cursorCol < token.length)
            return displayText;
    ...
    render(width) {
        ...
        // Prime Agent TUI: when a background color is configured, the editor renders
        // as a full-width dark surface block (no top/bottom borders, padding >= 2)
        // with a single prompt prefix (default "> ").
        ...
        // The prompt prefix is drawn inside the left padding region, so the text
        // and popover area must be narrowed by prefixWidth to keep every line
        // exactly `width` (a wider line crashes the regular-mode TuiMainScreen).
        const textWidth = Math.max(1, contentWidth - promptPrefixWidth);
        // Layout width: with padding the cursor can overflow into it,
        // without padding we reserve 1 column for the cursor.
        const layoutWidth = Math.max(1, textWidth - (paddingX ? 0 : 1));
        ...
        // Apply the surface background per ANSI-reset segment so the block survives
        // embedded resets (cursor, command color) — matches Prime's editor.
        const renderSurfaceLine = (line) => {
            ...
            return padded
                .split("\x1b[0m")
                .map((segment) => this.backgroundColor(segment))
                .join("\x1b[0m");
        };

        const popupBackgroundColor =
            this.autocompleteBackgroundColor ?? (useBackgroundSurface ? this.backgroundColor : undefined);
        // Prime Agent TUI: render the slash-command popover directly above the input
        // box, as a distinct panel with its own popup background.
        if (this.autocompleteState && this.autocompleteList) {
            const popupLeftPadding = " ".repeat(paddingX + promptPrefixWidth);
            const popupRightPadding = " ".repeat(paddingX);
            for (const line of ["", ...this.autocompleteList.render(textWidth), ""]) {
                const lineWidth = visibleWidth(line);
        ...
        const promptPrefix = promptPrefixWidth > 0
            ? this.formatPromptPrefix(truncateToWidth(promptPrefixText, promptPrefixWidth, ""))
            : "";
        for (let index = 0; index < visibleLines.length; index++) {
            const layoutLine = visibleLines[index];
            const absoluteLineIndex = this.scrollOffset + index;
            ...
            if (layoutLine.hasCursor && layoutLine.cursorPos !== undefined) {
                ...
                if (after.length > 0) {
                    ...
                    if (lineVisibleWidth > textWidth && paddingX > 0) {
            ...
            const padding = " ".repeat(Math.max(0, textWidth - lineVisibleWidth));
